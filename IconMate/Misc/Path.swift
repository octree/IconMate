...
extension FS.Path: ExpressibleByStringLiteral {
...
extension FS.Path: CustomStringConvertible {
...
extension FS.Path {
    ...
    public var isAbsolute: Bool {
        return path.hasPrefix(FS.Path.separator)
    }
    ...
    ///
    public func absolute() -> FS.Path {
        if isAbsolute {
            return normalize()
        }

        let expandedPath = FS.Path(NSString(string: self.path).expandingTildeInPath)
        if expandedPath.isAbsolute {
            return expandedPath.normalize()
        }

        return (FS.Path.current + self).normalize()
    }
    ...
    ///   representation.
    ///
    public func normalize() -> FS.Path {
        return FS.Path(NSString(string: self.path).standardizingPath)
    }
    ...
    ///
    public func symlinkDestination() throws -> FS.Path {
        let symlinkDestination = try FS.Path.fileManager.destinationOfSymbolicLink(atPath: path)
        let symlinkPath = FS.Path(symlinkDestination)
...
internal protocol FileSystemInfo {
    func isFSCaseSensitiveAt(path: FS.Path) -> Bool
}
...
extension FS.Path {
    ...
    public var exists: Bool {
        return FS.Path.fileManager.fileExists(atPath: self.path)
    }
    ...
    public var isDirectory: Bool {
        ...
        guard FS.Path.fileManager.fileExists(atPath: normalize().path, isDirectory: &directory) else {
            return false
        }
    ...
    public var isSymlink: Bool {
        do {
            _ = try FS.Path.fileManager.destinationOfSymbolicLink(atPath: path)
            return true
        } catch {
            return false
        }
    ...
    public var isReadable: Bool {
        return FS.Path.fileManager.isReadableFile(atPath: self.path)
    }
    ...
    public var isWritable: Bool {
        return FS.Path.fileManager.isWritableFile(atPath: self.path)
    }
    ...
    public var isExecutable: Bool {
        return FS.Path.fileManager.isExecutableFile(atPath: self.path)
    }
    ...
    public var isDeletable: Bool {
        return FS.Path.fileManager.isDeletableFile(atPath: self.path)
    }
}
...
extension FS.Path {
    ...
    public func mkdir() throws {
        try FS.Path.fileManager.createDirectory(atPath: self.path, withIntermediateDirectories: false, attributes: nil)
    }
    ...
    public func mkpath() throws {
        try FS.Path.fileManager.createDirectory(atPath: self.path, withIntermediateDirectories: true, attributes: nil)
    }
    ...
    public func delete() throws {
        try FS.Path.fileManager.removeItem(atPath: self.path)
    }
    ...
    ///
    public func move(_ destination: FS.Path) throws {
        try FS.Path.fileManager.moveItem(atPath: self.path, toPath: destination.path)
    }
    ...
    ///   directory in its new location.
    ///
    public func copy(_ destination: FS.Path) throws {
        try FS.Path.fileManager.copyItem(atPath: self.path, toPath: destination.path)
    }
    ...
    ///
    public func link(_ destination: FS.Path) throws {
        try FS.Path.fileManager.linkItem(atPath: self.path, toPath: destination.path)
    }
    ...
    ///
    public func symlink(_ destination: FS.Path) throws {
        try FS.Path.fileManager.createSymbolicLink(atPath: self.path, withDestinationPath: destination.path)
    }
}
...
extension FS.Path {
    ...
    ///
    public static var current: FS.Path {
        get {
            return self.init(FS.Path.fileManager.currentDirectoryPath)
        }
        set {
            _ = FS.Path.fileManager.changeCurrentDirectoryPath(newValue.description)
        }
    }
    ...
    public func chdir(closure: () throws -> Void) rethrows {
        let previous = FS.Path.current
        FS.Path.current = self
        defer { FS.Path.current = previous }
        try closure()
    }
}
...
extension FS.Path {
    ...
    ///
    public static var home: FS.Path {
        return FS.Path(NSHomeDirectory())
    }

    /// - Returns: the path of the temporary directory for the current user.
    ///
    public static var temporary: FS.Path {
        return FS.Path(NSTemporaryDirectory())
    }

    /// - Returns: the path of a temporary directory unique for the process.
    /// - Note: Based on `NSProcessInfo.globallyUniqueString`.
    ///
    public static func processUniqueTemporary() throws -> FS.Path {
    ...
    ///
    public static func uniqueTemporary() throws -> FS.Path {
...
extension FS.Path {
    ...
    ///
    public func children() throws -> [FS.Path] {
        return try FS.Path.fileManager.contentsOfDirectory(atPath: path).map {
            self + FS.Path($0)
        }
    }
    ...
    ///   any subdirectory.
    ///
    public func recursiveChildren() throws -> [FS.Path] {
        return try FS.Path.fileManager.subpathsOfDirectory(atPath: path).map {
            self + FS.Path($0)
        }
    }
}
...
// MARK: Globbing
extension FS.Path {
    public static func glob(_ pattern: String) -> [FS.Path] {
        var gt = glob_t()
    ...
    public func glob(_ pattern: String) -> [FS.Path] {
        return FS.Path.glob((self + pattern).description)
    }
}

// MARK: SequenceType
extension FS.Path: Sequence {
    ...
    public struct PathSequence: Sequence {
        private var path: FS.Path
        private var options: DirectoryEnumerationOptions
        init(path: FS.Path, options: DirectoryEnumerationOptions) {
            self.path = path
    ...
    public struct DirectoryEnumerator: IteratorProtocol {
        public typealias Element = FS.Path

        let path: FS.Path
        let directoryEnumerator: FileManager.DirectoryEnumerator?

        init(path: FS.Path, options mask: DirectoryEnumerationOptions = []) {
            let options = FileManager.DirectoryEnumerationOptions(rawValue: mask.rawValue)
            self.path = path
            self.directoryEnumerator = FS.Path.fileManager.enumerator(at: path.url, includingPropertiesForKeys: nil, options: options)
        }

        public func next() -> FS.Path? {
            let next = directoryEnumerator?.nextObject()

            if let next = next as? URL {
                return FS.Path(next.path)
            }
            return nil
        }
...
///
public func ==(lhs: FS.Path, rhs: FS.Path) -> Bool {
...
public func +(lhs: FS.Path, rhs: FS.Path) -> FS.Path {
    return lhs.path + rhs.path
}

/// Appends a String fragment to another Path to produce a new Path
public func +(lhs: FS.Path, rhs: String) -> FS.Path {
    return lhs.path + rhs
}

/// Appends a String fragment to another String to produce a new Path
private func +(lhs: String, rhs: String) -> FS.Path {
    if rhs.hasPrefix(FS.Path.separator) {
        ...
        if lSlice.count > 1 && lSlice.last == FS.Path.separator {
            lSlice.removeLast()
        }
        ...
        while lSlice.last != ".." && !lSlice.isEmpty && rSlice.first == ".." {
            if lSlice.count > 1 || lSlice.first != FS.Path.separator {
        ...
        return FS.Path(components: lSlice + rSlice)
    }
}
...
public extension FS.Path {
    static var document: FS.Path {
        let path = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0]
        return FS.Path(path)
    }

    static func path(forAppGroup identifier: String) -> FS.Path? {
        return FileManager.default
            .containerURL(forSecurityApplicationGroupIdentifier: identifier)
            .map { FS.Path($0.path) }
    }

    static var cache: FS.Path {
        let path = NSSearchPathForDirectoriesInDomains(.cachesDirectory,
                                                       .userDomainMask, true)[0]
        return FS.Path(path)
    }

    var deletingLastComponent: FS.Path {
        var arr = components
        arr.removeLast()
        return FS.Path(components: arr)
    }
}
